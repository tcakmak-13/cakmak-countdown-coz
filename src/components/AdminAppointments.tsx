...
export default function AdminAppointments() {
  ...
  const handleUpdateTime = async () => {
    ...
    if (error) toast.error('Güncelleme hatası: ' + error.message);
      ...
      toast.success('Randevu güncellemeleri tüm gelecek haftalara yansıtıldı!');
      setEditDialog(false);
      fetch_();
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Haftalık tekrarlanan görüşme taleplerini yönet</p>
        </div>
      </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1 text-amber-400">
                    ...
                    <span className="text-xs font-medium">Haftalık Tekrar Talebi</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-start gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => endSeries(a.id)}
                      disabled={updatingId === a.id}
                      className="gap-1 border-destructive/30 text-destructive hover:bg-destructive/10"
                    >
                      {updatingId === a.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <StopCircle className="h-3.5 w-3.5" />}
                      Seriyi Sonlandır
                    </Button>
                  </div>
                </div>
          ...
          {rest.slice(0, 10).map(a => (
            <div key={a.id} className="glass-card rounded-xl p-4 flex items-center gap-3 opacity-60">
              ...
              <span className="text-sm flex-1">
                {a.profiles?.full_name} — {DAY_NAMES[a.recurring_day ?? 0]} {a.recurring_time}
              </span>
